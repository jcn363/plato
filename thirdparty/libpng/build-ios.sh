...
BUILD_DIR=$PROJECT_ROOT/target/libpng/iOS/arm64
mkdir -p $BUILD_DIR
...
./configure --prefix=$BUILD_DIR --host=arm-apple-darwin --disable-shared
make clean || true
...
BUILD_DIR=$PROJECT_ROOT/target/libpng/iOS-sim/arm64
mkdir -p $BUILD_DIR
...
BUILD_DIR=$PROJECT_ROOT/target/libpng/iOS-sim/x86_64
mkdir -p $BUILD_DIR
...
./configure --prefix=$BUILD_DIR --host=x86_64-apple-darwin --disable-shared
make clean || true
make -j$(sysctl -n hw.ncpu)
make install
...
mkdir -p $PROJECT_ROOT/target/libpng/iOS-device/lib
mkdir -p $PROJECT_ROOT/target/libpng/iOS-simulator/lib
lipo -create \
  $PROJECT_ROOT/target/libpng/iOS-sim/arm64/lib/libpng16.a \
  $PROJECT_ROOT/target/libpng/iOS-sim/x86_64/lib/libpng16.a \
  -output $PROJECT_ROOT/target/libpng/iOS-simulator/lib/libpng16.a || echo "Some simulator architectures may have failed"
cp $PROJECT_ROOT/target/libpng/iOS/arm64/lib/libpng16.a $PROJECT_ROOT/target/libpng/iOS-device/lib/
