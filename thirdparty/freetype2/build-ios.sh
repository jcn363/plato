...
./configure --prefix=$BUILD_DIR --host=arm-apple-darwin --without-harfbuzz --enable-static
make clean || true
...
./configure --prefix=$BUILD_DIR --host=x86_64-apple-darwin --without-harfbuzz --enable-static
make clean || true
make -j$(sysctl -n hw.ncpu)
make install
