...
macro_rules! hb_tag {
    ($c1:expr, $c2:expr, $c3:expr, $c4:expr) => {
        ($c1 as HbTag) << 24 | ($c2 as HbTag) << 16 | ($c3 as HbTag) << 8 | ($c4 as HbTag)
    };
}
...
impl Default for HbFeature {
    fn default() -> Self {
        // SAFETY: HbFeature is a repr(C) struct with all-Copy fields. mem::zeroed() produces a valid state.
        unsafe { mem::zeroed() }
    }
}
