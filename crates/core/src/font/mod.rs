...
impl FontOpener {
    ...
    pub fn open_memory(&self, buf: &[u8]) -> Result<Font, Error> {
        // SAFETY: FFI call to FreeType with a valid library pointer and font buffer slice.
        unsafe {
            let mut face = ptr::null_mut();
...
impl Font {
    pub fn family_name(&self) -> Option<&str> {
        // SAFETY: FFI call to FreeType. Face pointer is valid. family_name is checked for null.
        unsafe {
    ...
    pub fn style_name(&self) -> Option<&str> {
        // SAFETY: FFI call to FreeType. Face pointer is valid. style_name is checked for null.
        unsafe {
    ...
    pub fn set_size(&mut self, size: u32, dpi: u16) {
        ...
        // SAFETY: FFI calls to FreeType and HarfBuzz with valid face and font pointers.
        unsafe {
    ...
    pub fn set_variations(&mut self, specs: &[&str]) {
        ...
        unsafe {
            ...
            let ret = FT_Set_Var_Design_Coordinates(
                self.face,
                coords.len() as libc::c_uint,
                coords.as_ptr(),
            );
        ...
        unsafe {
            ...
            'outer: for i in 0..styles_count {
                ...
                for j in 0..names_count {
                    ...
                    let vec_ascii: Vec<u8> = slice
                        .iter()
                        .enumerate()
                        .filter_map(|x| if x.0 % 2 == 0 { None } else { Some(*x.1) })
                        .collect();
    ...
    unsafe fn patch(
        ...
        // SAFETY: FFI calls to HarfBuzz and FreeType with valid pointers. Buffer and font pointers are owned by self.
        let mut drift = 0;
    ...
    pub fn plan<S: AsRef<str>>(
        ...
        // SAFETY: FFI calls to HarfBuzz for text shaping and FreeType for glyph metrics. Font pointer is valid.
        unsafe {
            let buf = hb_buffer_create();
            hb_buffer_add_utf8(
                buf,
                text.as_ref().as_ptr() as *const libc::c_char,
                text.as_ref().len() as libc::c_int,
                0,
                -1,
            );
                ...
                .map(|ftr| {
                    ftr.iter()
                        .filter_map(|f| {
                            ...
                            let ret = hb_feature_from_string(
                                f.as_ptr() as *const libc::c_char,
                                f.len() as libc::c_int,
                                &mut feature,
                            );
                            if ret == 1 {
            ...
            hb_shape(
                ...
                buf,
                features_vec.as_ptr(),
                features_vec.len() as libc::c_uint,
            );
    ...
    pub fn crop_around(&self, render_plan: &mut RenderPlan, index: usize, max_width: i32) -> usize {
        ...
        if lower_index >= 0 {
            ...
            render_plan.glyphs = self
                .ellipsis
                .glyphs
                .iter()
                .chain(render_plan.glyphs[lower_index as usize..].iter())
                .cloned()
                .collect();
        }
        ...
        render_plan
            .scripts
            .retain(|&k, _| k >= lower_index.max(0) as usize && k <= upper_index);
        if lower_index > 0 {
            render_plan.scripts = render_plan
                .scripts
                .drain()
                .map(|(k, v)| (k - lower_index as usize + 1, v))
                .collect();
        }
    ...
    pub fn render(
        ...
        // SAFETY: FFI calls to FreeType for glyph rendering. Face and glyph slot pointers are valid. Bitmap buffer is dereferenced within bounds.
        unsafe {
            let mut pos = origin;
            ...
            for (index, glyph) in render_plan.glyphs.iter().enumerate() {
                let face = if let Some(script) = render_plan.scripts.get(&index) {
                    *fallback_faces.entry(script).or_insert_with(|| {
                        ...
                        FT_Set_Pixel_Sizes(
                            face,
                            (*(*self.face).size).metrics.x_ppem as libc::c_uint,
                            0,
                        );
                        face
                    })
                ...
                let top_left =
                    pos + glyph.offset + pt!((*glyph_slot).bitmap_left, -(*glyph_slot).bitmap_top);
    ...
    pub fn height(&self, c: char) -> u32 {
        // SAFETY: FFI call to FreeType with valid face pointer.
        unsafe {
    ...
    #[inline]
    pub fn em(&self) -> u16 {
        // SAFETY: FFI call to FreeType. Face and size pointers are valid and initialized.
...
impl Drop for Font {
    fn drop(&mut self) {
        // SAFETY: FFI calls to FreeType and HarfBuzz. Face and font pointers were initialized during Font creation.
        unsafe {
