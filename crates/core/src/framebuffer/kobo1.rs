...
impl KoboFramebuffer1 {
    pub fn new<P: AsRef<Path>>(path: P) -> Result<KoboFramebuffer1, Error> {
        ...
        // SAFETY: File descriptor from the framebuffer device is valid. Memory mapping
        // parameters are validated by the kernel.
        let frame = unsafe {
            libc::mmap(
                ptr::null_mut(),
...
impl Framebuffer for KoboFramebuffer1 {
    ...
    fn update(&mut self, rect: &Rectangle, mode: UpdateMode) -> Result<u32, Error> {
        ...
        let result = if mark >= 11 {
            ...
            // SAFETY: File descriptor is valid. Update data is properly initialized.
    ...
    fn wait(&self, token: u32) -> Result<i32, Error> {
        let result = if CURRENT_DEVICE.mark() >= 7 {
            ...
            // SAFETY: File descriptor is valid. Marker data is properly initialized.
            unsafe { wait_for_update_v2(self.file.as_raw_fd(), &mut marker_data) }
        } else {
            // SAFETY: File descriptor is valid. Token pointer refers to a valid stack value.
    ...
    fn set_rotation(&mut self, n: i8) -> Result<(u32, u32), Error> {
        ...
        for (i, v) in [n, (n + 1) % 4, n].iter().enumerate() {
            ...
            // SAFETY: File descriptor is valid. var_info is a properly initialized VarScreenInfo.
            let result =
    ...
    fn set_inverted(&mut self, enable: bool) {
        ...
        if CURRENT_DEVICE.mark() < 11 {
            ...
            OpenOptions::new()
                ...
                .map_err(|e| log_error!("Failed to invert colors: {:#?}", e))
                .ok();
        }
    }
...
fn set_pixel_rgb_8(fb: &mut KoboFramebuffer1, x: u32, y: u32, rgb: [u8; 3]) {
    ...
    // SAFETY: Offset is bounds-checked by the debug_assert above and is within frame bounds.
    unsafe {
