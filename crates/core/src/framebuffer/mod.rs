...
pub trait Framebuffer {
    ...
    fn draw_framed_pixmap_contrast_alpha(
        &mut self,
        pixmap: &Pixmap,
        rect: &Rectangle,
        pt: Point,
        ...
        if (exponent - 1.0).abs() < f32::EPSILON && alpha == 255 {
            self.draw_framed_pixmap(pixmap, rect, pt);
            return;
        }
        let rem_gray = 255.0 - gray;
        ...
        for y in rect.min.y..rect.max.y {
            for x in rect.min.x..rect.max.x {
                ...
                let color = raw_color.apply(|comp| {
                    let c = comp as f32;
                    let adjusted = if c < gray {
                        gray * (c / gray).powf(exponent)
                    } else if c > gray {
                        gray + rem_gray * ((c - gray) / rem_gray).powf(inv_exponent)
                    } else {
                        gray
                    };
                    let alpha_factor = alpha as f32 / 255.0;
                    (adjusted * alpha_factor) as u8
                });
                self.set_pixel(px as u32, py as u32, color);
            }
        }
