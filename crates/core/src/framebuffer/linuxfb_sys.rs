...
impl Default for Bitfield {
    fn default() -> Self {
        // SAFETY: Bitfield is a repr(C) struct with only integer fields; zeroed memory is valid.
        unsafe { mem::zeroed() }
    }
}
...
impl Default for VarScreenInfo {
    fn default() -> Self {
        // SAFETY: VarScreenInfo is a repr(C) struct with only integer fields; zeroed memory is valid.
        unsafe { mem::zeroed() }
    }
}
...
impl Default for FixScreenInfo {
    fn default() -> Self {
        // SAFETY: FixScreenInfo is a repr(C) struct with only integer/fixed-size fields; zeroed memory is valid.
        unsafe { mem::zeroed() }
    }
}
...
pub fn fix_screen_info(file: &File) -> Result<FixScreenInfo, Error> {
    ...
    // SAFETY: File descriptor is valid. ioctl parameters match the expected kernel interface.
