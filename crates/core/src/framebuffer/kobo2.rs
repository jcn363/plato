...
impl KoboFramebuffer2 {
    pub fn new<P: AsRef<Path>>(path: P) -> Result<KoboFramebuffer2, Error> {
        ...
        let ion = File::open("/dev/ion").with_context(|| "can't open ion device")?;
        ...
        // SAFETY: Ion device file descriptor is valid. Allocation parameters are validated.
        let result = unsafe { ion_alloc(ion.as_raw_fd(), &mut data) };
        ...
        // SAFETY: Ion device file descriptor is valid. Mapping parameters are validated.
        let result = unsafe { ion_map(ion.as_raw_fd(), &mut data) };

        if let Err(e) = result {
            let mut data = IonHandleData {
                handle: data.handle,
            };
        ...
        // SAFETY: File descriptor from ion_map is valid. Memory mapping parameters are validated by the kernel.
        let frame = unsafe {
            libc::mmap(
                ptr::null_mut(),
                alloc_size,
        ...
        let layer = DispLayerConfig2 {
            info: DispLayerInfo2 {
                ...
                out_trd_mode: 0,
                color_fb: ColorFb {
                    fb: ManuallyDrop::new(DispFbInfo2 {
                        fd: 0,
                        y8_fd: data.fd,
                        size: [
                            DispRectsz {
    ...
    fn as_bytes(&self) -> &[u8] {
        // SAFETY: Frame pointer and size are initialized during construction and remain valid.
    ...
    fn get_pixel(&self, x: u32, y: u32) -> Color {
        ...
        // SAFETY: Offset is computed from valid screen dimensions and is within frame bounds.
...
impl Framebuffer for KoboFramebuffer2 {
    ...
    fn set_pixel(&mut self, x: u32, y: u32, color: Color) {
        ...
        // SAFETY: Spot pointer is derived from a valid frame offset.
        unsafe { *spot = c.gray() };
    }
    ...
    fn wait(&self, token: u32) -> Result<i32, Error> {
        let marker_data = SunxiDispEinkWaitFrameSyncComplete { frame_id: token };
        // SAFETY: Display file descriptor is valid. Marker data is stack-allocated and valid.
        let result = unsafe { wait_for_update(self.display.as_raw_fd(), &marker_data) };
    ...
    fn set_rotation(&mut self, n: i8) -> Result<(u32, u32), Error> {
        ...
        if delta % 2 == 1 {
            ...
            mem::swap(
                &mut self.var_info.xres_virtual,
                &mut self.var_info.yres_virtual,
            );
            mem::swap(
                &mut self.layer.info.screen_win.width,
                &mut self.layer.info.screen_win.height,
            );
            // SAFETY: The ManuallyDrop<DispFbInfo2> pointer is valid and was initialized during construction.
            unsafe {
                let rect = &mut (*self.layer.info.color_fb.fb).size[0];
                mem::swap(&mut rect.width, &mut rect.height);
...
impl Drop for KoboFramebuffer2 {
    fn drop(&mut self) {
        // SAFETY: Frame pointer, alloc_size, file descriptors, and ion handles were all
        // initialized during construction and are valid for the lifetime of this struct.
        unsafe {
            libc::munmap(self.frame, self.alloc_size);
            libc::close(self.fd_data.fd);
            let mut data = IonHandleData {
                handle: self.fd_data.handle,
            };
