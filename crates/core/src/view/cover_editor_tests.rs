...
mod crop_selection_tests {
    ...
    use crate::view::cover_editor::{
        CropState, EditorMode, CROP_BORDER_THICKNESS, CROP_SELECTION_COLOR, MIN_CROP_SIZE,
    };
    ...
    fn test_minimum_selection_size() {
        ...
        assert!(small_rect.width() < MIN_CROP_SIZE);
        assert!(small_rect.height() < MIN_CROP_SIZE);
        assert!(valid_rect.width() >= MIN_CROP_SIZE);
        assert!(valid_rect.height() >= MIN_CROP_SIZE);
    }
