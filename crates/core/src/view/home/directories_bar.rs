...
impl DirectoriesBar {
    ...
    fn make_children(
        ...
        let mut pos = pt!(
            self.rect.min.x + small_half(padding),
        ...
        let filler = Filler::new(
            rect![
                self.rect.min,
                pt!(self.rect.max.x, self.rect.min.y + small_half(baselines[0]))
            ],
            background,
        );
        ...
        let filler = Filler::new(
            rect![
                pt!(self.rect.min.x, self.rect.min.y + small_half(baselines[0])),
                pt!(
                    self.rect.min.x + small_half(padding),
        ...
        for (line_index, line) in page.lines.iter().enumerate() {
            let paddings = if line_index == lines_count - 1 && page.end_index == directories_count {
                ...
                let horizontal_space =
                    (line.items.len() as i32 - 1) * padding + max_line_width - line.width;
            ...
            let rect_height =
                big_half(baselines[line_index]) + x_height + small_half(baselines[line_index + 1]);
            ...
            for (item_index, item) in line.items.iter().enumerate() {
                ...
                match *item {
                    ...
                    Item::Icon { name, .. } => {
                        let dir = if item_index == 0 {
                            CycleDir::Previous
                        } else {
                            CycleDir::Next
                        };
                        let child = Icon::new(name, rect![pos, sop], Event::Page(dir))
        ...
        if page.end_index == directories_count {
            ...
            let y_offset = baselines.iter().take(lines_count).sum::<i32>()
                - big_half(baselines[lines_count - 1])
                + (lines_count - 1) as i32 * x_height;
            let height = big_half(baselines[lines_count - 1])
                + x_height
                + small_half(baselines[lines_count]);
            let filler = Filler::new(
                rect![
                    pt!(self.rect.min.x + x_offset, self.rect.min.y + y_offset),
                    pt!(
                        self.rect.max.x - big_half(padding),
                        self.rect.min.y + y_offset + height
                    )
                ],
                background,
            );
        ...
        if lines_count < max_lines {
            let y_offset = baselines.iter().take(lines_count + 1).sum::<i32>()
                - big_half(baselines[lines_count])
                + lines_count as i32 * x_height;
            let filler = Filler::new(
                rect![
                    pt!(
                        self.rect.min.x + small_half(padding),
