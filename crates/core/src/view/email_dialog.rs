...
impl EmailDialog {
    pub fn new(context: &mut Context, document_path: Option<PathBuf>) -> EmailDialog {
        ...
        let input_to = InputField::new(rect_input_to, ViewId::EmailSubjectInput)
            .border(true)
        ...
        let label_subject = Label::new(
            rect_label_subject,
            LABEL_SUBJECT.to_string(),
            Align::Left(0),
        );
        ...
        let input_subject =
            InputField::new(rect_input_subject, ViewId::EmailSubjectInput).border(true);
        ...
        // Add Cancel button
        let _button_width = font.plan(LABEL_CANCEL, Some(max_button_width), None).width + padding;
        ...
        // Add Send button
        let send_button_width = font.plan(LABEL_SEND, Some(max_button_width), None).width + padding;
        let rect_send = rect![
