...
use crate::document::{BoundedText, Boundary, CycleDir, Location, TocEntry};
...
impl DjvuDocument {
    ...
    pub fn new<P: AsRef<Path>>(path: P) -> Result<Self, Error> {
        ...
        // Validate path
        if !path.exists() {
            return Err(Error::msg(format!("DJVU file not found: {}", path.display())));
        }
        
        // Read DJVU file data
        let data = std::fs::read(path)
            .context("Failed to read DJVU file")?;
        
        // Parse DJVU document using djvu-rs
        let inner = djvu_rs::DjVuDocument::parse(&data)
            .context("Failed to parse DJVU document")?;
