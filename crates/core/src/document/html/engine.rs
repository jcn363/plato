...
impl Engine {
    ...
    pub fn build_display_list(
        ...
        style.font_size = style.font_size.max(self.min_font_size);
        ...
        style.tab_size = props
            .get("tab-size")
            .and_then(|value| parse_tab_size(value, style.font_size, self.dpi))
        ...
        if node.parent().is_some() {
            ...
            style.border = parse_edge(
                props.get("border-top").map(String::as_str),
                props.get("border-right").map(String::as_str),
                props.get("border-bottom").map(String::as_str),
                props.get("border-left").map(String::as_str),
                style.font_size,
                self.font_size,
                parent_style.width,
                self.dpi,
        ...
        style.text_overflow = props
            .get("text-overflow")
            .and_then(|value| parse_text_overflow(value));
        ...
        if has_blocks {
            ...
            if node.has_children() {
                ...
                if node.tag_name() == Some("hr") {
                    let line_rect = rect![
                        style.start_x,
                        draw_state.position.y + style.margin.top,
                        style.end_x,
                        draw_state.position.y + style.margin.top + 2,
                    ];
                    rects[0] = Some(line_rect);
                    display_list
                        .last_mut()
                        .expect("display list is empty")
                        .push(DrawCommand::ExtraRect(line_rect));
                    draw_state.position.y += style.margin.top + 2 + style.margin.bottom;
