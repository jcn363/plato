...
use crate::settings::{ExternalStorageSettings, ImportSettings, LibraryMode, LibraryStatistics};
...
impl Library {
    ...
    pub fn add_document(&mut self, info: Info) {
        ...
        let Ok(md) = path.metadata() else { return };
        let Ok(fp) = md.fingerprint(self.fat32_epoch) else {
            return;
        };
    ...
    /// Helper to get fingerprint from path, using cache when available.
    /// Consolidates redundant logic from sync_reader_info, thumbnail_preview, set_status.
    fn get_fingerprint(&self, path: &Path) -> Fp {
