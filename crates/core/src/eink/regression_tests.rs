...
fn test_damage_checkerboard_pattern() {
    ...
    assert!(
        total_damage > 1000,
        "Checkerboard should have significant damage"
    );
}
...
fn test_gamma_extreme_values() {
    ...
    let high_gamma =
        GrayscaleConverter::with_gamma(DitheringMode::None, 5.0).expect("Test assertion failed");
    let result = high_gamma
        .convert(&mid_gray, 1, 1)
        .expect("Test assertion failed");
    // Gamma 5.0: 128 -> ~196 -> ~12 in 16-level (brightened from mid-gray ~7-8)
    assert!(
        result[0] >= 11,
        "High gamma should brighten mid-tones, got {}",
        result[0]
    );

    // Low gamma (<1.0) darkens mid-tones (curve becomes convex)
    let low_gamma =
        GrayscaleConverter::with_gamma(DitheringMode::None, 0.5).expect("Test assertion failed");
    let result = low_gamma
        .convert(&mid_gray, 1, 1)
        .expect("Test assertion failed");
    // Gamma 0.5: 128 -> ~52 -> ~3 in 16-level (darkened from mid-gray ~7-8)
    assert!(
        result[0] <= 5,
        "Low gamma should darken mid-tones, got {}",
        result[0]
    );
}
...
fn test_ghosting_reducer_max_updates_boundary() {
    ...
    assert!(
        reducer.should_force_full_refresh(),
        "Should force refresh at max"
    );
    ...
    assert!(
        !reducer.should_force_full_refresh(),
        "Should not force refresh below max"
    );
}
...
fn test_waveform_all_combinations() {
    ...
    let update_types = [UpdateType::Full, UpdateType::Partial, UpdateType::Fast];
...
fn test_controller_input_validation() {
    ...
    assert!(sunxi
        .update(region, &empty_data, WaveformMode::GC16)
        .is_err());
    ...
    assert!(sunxi
        .update(region, &valid_data, WaveformMode::GC16)
        .is_err());
