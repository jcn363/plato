...
fn update_token(client: &Client, session: &mut Session, settings: &Settings) -> Result<(), Error> {
    ...
    let response = client.post(&url).json(&query).send()?;
...
fn main() -> Result<(), Error> {
        ...
        for line in contents.lines() {
            ...
            if !response.status().is_success() {
                ...
                let err_desc = body
                    ...
                    .or_else(|| status.canonical_reason())
                    .unwrap_or_else(|| status.as_str());
                log_error!("Can't add {}: {}.", line, err_desc);
            }
        }
    }
        ...
        if let Ok(event) = serde_json::from_str::<JsonValue>(&line) {
            if let Some(results) = event.get("results").and_then(JsonValue::as_array) {
                            ...
                            .and_then(|v| v.parse::<u64>().ok())
                        {
                            ...
                            if response.status().is_success() {
                                ...
                                let err_desc = body
                                    ...
                                    .or_else(|| status.canonical_reason())
                                    .unwrap_or_else(|| status.as_str());
                                log_error!("Can't mark {} as read: {}.", id, err_desc);
                            }
                        }
                    }

                    if settings.remove_finished {
                        if let Some(path) = entry.pointer("/file/path").and_then(JsonValue::as_str)
                        {
                            let event = json!({
                                "type": "removeDocument",
                                "path": path,
                ...
                if !results.is_empty() {
                    ...
                    if settings.remove_finished {
                        let removals_count =
                            session.removals_count.saturating_sub(last_removals_count);
            ...
            for element in items {
                if sigterm.load(Ordering::Relaxed)
                    || (settings.balance_limit > 0
                        && session
                            .downloads_count
                            .saturating_sub(session.removals_count)
                ...
                if let Err(err) = response {
                    log_error!("Can't download {}: {:#}.", id, err);
    ...
    if pages_count > 0 {
        let downloads_count = session.downloads_count.saturating_sub(last_downloads_count);
